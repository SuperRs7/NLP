...
from Chapter_05.LLaMA2 import ModelConfig,Transformer
from Chapter_05.dataset import PretrainDataset
...
def get_lr(it, all):
    ...
    lr_dacay_iters = all # 学习率衰减的总迭代次数
    min_lr = args.learning_rate / 10 # 最小学习率， 为初始学习率的 1 / 10

    # Warmup 截断，线性增长
    if it < warmup_iters:
        return args.learning_rte * it / warmup_iters

    # 超出训练步数： 保持最小学习率
    if it > lr_dacay_iters:
        return min_lr
    # 余弦退火阶段
    decay_ratio = (it - warmup_iters) / (lr_dacay_iters - warmup_iters)
...
if __name__ == "__main__":
    ...
    # 基础训练参数
    parser.add_argument("--out_dir", typer=str, default="base_model_215M", help="模型输出目录")
    ...
    # 实验跟踪和数据加载参数
    parser.add_argument("--use-swanlab", action="store_true", help="是否使用SwanLab进行实验跟踪")
    parser.add_argument("--num_workers", type=int, default=8, help="数据加载的工程进度数")
    parser.add_argument("--data_path", type=str, default="./seq_monkey_datawhale.jsonl", help="训练数据路径")

    # 训练优化参数
    parser.add_argument("--accumulation_steps", type=int, default=8, help="训练累积步数")
    ...
    train_loader = DataLoader(
        ...
        shuffle=True, # 随机打乱数据
        num_workers=args.num_workers # 数据假造的并行工作进程数
    )
    # ==================== 优化器和训练组件初始化 ====================
    # 初始化混合精度训练的梯度缩放器
